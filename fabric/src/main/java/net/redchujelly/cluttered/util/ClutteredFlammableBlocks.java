...
import net.redchujelly.cluttered.mixin.FireBlockInvoker;
...
public class ClutteredFlammableBlocks {
    ...
    public static void addFlammables(){
        ...
        for (Map.Entry<Block, Tuple<Integer, Integer>> entry : FLAMMABLES.entrySet()){
            ((FireBlockInvoker) fireBlock).cluttered_invokeSetFlammable(entry.getKey(), entry.getValue().getA(), entry.getValue().getB());
        }
    }
}
